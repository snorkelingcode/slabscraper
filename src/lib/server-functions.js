...
import { fetchWithBrowserHeaders, fetchWithScrapingService } from './http-util';
...
export async function fetchHtml(url) {
  try {
    ...
    let result = await fetchWithBrowserHeaders(url);
    
    // If that fails and scraping service is enabled, try that
    if (result.error && process.env.USE_SCRAPING_SERVICE === 'true') {
      console.log('Browser header fetch failed, trying scraping service...');
      result = await fetchWithScrapingService(url);
    }
    ...
    return { data: result.html, error: null };
  } catch (error) {
    return {
      data: null,
